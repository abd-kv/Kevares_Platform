...
def calculate_distance(x1, y1, x2, y2):
    x1_rad = math.radians(x1)  # Convert x1 to radians
    dx = (x2 - x1) / math.cos(x1_rad)
    dy = y2 - y1
    return math.sqrt(dx**2 + dy**2)

def calculate_angle(x1, y1, x2, y2):
    x1_rad = math.radians(x1)  # Convert x1 from degrees to radians
    dx = (x2 - x1) / math.cos(x1_rad)
    dy = y2 - y1
...
class NavigationAndControlNode(Node):
    ...
    def navigate(self):
        ...
        if distance < 0.00010:  # Threshold for reaching the waypoint #< 0.000009
    ...
    def control_robot(self, alpha):
        ...
        # if the robot is making a hard turn (deviation > 20 degrees), reduce speed to 250;
        # otherwise, use 500 speed when moving relatively straight.
        if abs(clamped_alpha) > 20:
            speed = 300
        else:
            speed = 300
