...
class NavigationAndControlNode(Node):
    def __init__(self):
        ...
        self.cmd_vel_pub = self.create_publisher(Int32MultiArray, 'movement_commands', 10)
        ...
        self.waypoints = self.load_waypoints('/home/user/abd_ws_2/src/tests/tests/perimeter/23JunC/final_combined_waypoints.csv')
    ...
    def control_robot(self, steering_angle, obstacle_correction):
        ...
        speed_mapped = self.v * self.speed_factor
        steering_mapped = steering

        cmd = Int32MultiArray()
        cmd.data = [int(speed_mapped) , int(steering_mapped)]

        self.get_logger().info(
            f"[Control] Stanley: {steering_angle:.2f}°, Obstacle: {obstacle_correction:.2f}°, Final: {combined_steering_deg:.2f}° | Speed: {int(speed_mapped):.2f} m/s"
        )
