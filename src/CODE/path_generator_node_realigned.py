...
# -------------------------------
# Utility Function for Rotation
# -------------------------------
def rotate_polygon_to_axis(polygon):
    mrr = polygon.minimum_rotated_rectangle
    mrr_coords = list(mrr.exterior.coords)
    x_rad = math.radians(mrr_coords[0][0])
    dx = (mrr_coords[1][0] - mrr_coords[0][0])/math.cos(x_rad)
...
def inverse_rotate_coords(coords, angle_deg, origin):
    return [shapely_rotate(Point(p), angle_deg, origin=origin, use_radians=False).coords[0] for p in coords]
...
class PathGenerator(Node):
    ...
    def run(self):
        try:
            ...
            rotated_boundary, angle_deg = rotate_polygon_to_axis(boundary)
            centroid = boundary.centroid

            # Generate zigzag paths in rotated frame
            primary, secondary, tertiary = self.generate_primary_secondary_tertiary_paths(rotated_boundary)

            # Rotate paths back
            primary = inverse_rotate_coords(primary, angle_deg, centroid)
            secondary = inverse_rotate_coords(secondary, angle_deg, centroid)
            tertiary = inverse_rotate_coords(tertiary, angle_deg, centroid)
            ...
            self.plot_all_paths(boundary, primary, secondary, tertiary, coordinates)

            combined_path = []
            for bp in border_paths:
                combined_path.extend(bp)
            combined_path.extend(secondary)
            combined_path.extend(reversed(primary))
            combined_path.extend(tertiary)
