...
class RearLeftRealSensePublisher(Node):

    def __init__(self):
        super().__init__('rear_left_realsense_publisher_node')
        self.publisher_color = self.create_publisher(Image, 'rear_left_color_image', 10)
        self.publisher_depth = self.create_publisher(Image, 'rear_left_depth_image', 10)
        self.publisher_intrinsics = self.create_publisher(Int32MultiArray, 'rear_left_camera_info_array', 10)
        self.bridge = CvBridge()

        # Initialize RealSense pipeline for the left camera
        self.pipeline = rs.pipeline()
        self.config = rs.config()

        # Enable stream with the left camera's serial number
        serial_number_left = '211622060151'  # Replace this with your left camera's serial number
        self.config.enable_device(serial_number_left)
...
def main(args=None):
    ...
    node = RearLeftRealSensePublisher()
    rclpy.spin(node)
    node.destroy_node()
    rclpy.shutdown()
